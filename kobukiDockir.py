...
class Kobuki():
	...
	self.sub_odometry=rospy.Subscriber('/odom',Odometry,self.position,queue_size=10)
        ...
        self.dock_ir_flag=0
        self.angle=0
        self.angle_first_flag=0
        self.angle_first=0
    def position(self,odom):
        self.angle=0
        print("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa")
        if self.angle==odom.twist.twist.angular.z and self.angle_first_flag==0:
            self.angle_first_flag=1
            self.angle=odom.twist.twist.angular.z
            self.angle_first=self.angle
        elif not self.angle==odom.twist.twist.angular.z and self.angle_first_flag==1:
            self.angle=odom.twist.twist.angular.z
    def controller(self):
	        r = rospy.Rate(10.0)
		    ...
		    vel = Twist()
                    ...
                    if self.dock_ir_flag==0 and self.bump==0 and self.wheel_drop_flag==0 and not self.angle-self.angle_first<350:
                               vel.angular.z=self.vel_rot
                    elif self.bump==1 and self.wheel_drop_flag==0 and self.count<20:
		    		vel.linear.x=-0.1
